Csscoin Core version 0.13.2 is now available from:

  <https://download.csscoin.org/csscoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/csscoin-project/csscoin/issues>
...
updates will be released anymore. Without security updates, using a csscoin
...
ECDSA signatures inside Csscoin transactions now use validation using
...
Previous versions of Csscoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Csscoin Core was to raise the default minimum
relay fee.

Csscoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Csscoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running csscoind without having to do any manual configuration.
...
Csscoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Csscoin Core will relay transactions with
...
entirely in Csscoin Core 0.13, and it is at this time undecided whether the
...
Csscoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Csscoin Core automatically creates a hidden service to listen on, without
manual configuration. Csscoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Csscoin Core is listening, and
...
Csscoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Csscoin
Core to use floating fees. By default, Csscoin Core will use floating
fees.
...
(default: `0.0002` CSS/kB).

At all times, Csscoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CSS.
Furthermore, Csscoin Core will never create transactions paying less than
...
`csscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /csscoinrpc>
        ...
        # optional bypass csscoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< csscoinrpc:<password>
    </Location>
...
Restarting csscoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in csscoin.conf
...
For example:
    ...
    $ src/csscoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Csscoin Core code base has
...
- `csscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `csscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Csscoin scripting system that in combination with
...
Csscoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Csscoin Core.
...
The two phases are distinct in the Csscoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Csscoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Csscoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Csscoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of csscoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of csscoins being spent is exactly
...
- **Increased security for multisig:** Csscoin addresses (both P2PKH addresses
  ...
  believed to be provided by Csscoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Csscoin's current ledger (called the UTXO set),
  ...
  note that Csscoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Csscoin users to individually opt-in to almost any change in the Csscoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Csscoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Csscoin
client to Csscoin Core 0.13.2 to signal for segwit support. When a
...
Since Csscoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `csscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `csscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
